...
def load_job_from_db(jid):
  conn = engine.connect()
  result = conn.execute(
    text("SELECT * FROM jobs WHERE id = :jid"),{'jid': jid}
  )
  row = result.fetchone()
  if not row:
    return None
...
def add_application_to_db(job_id, data):
  ...
  conn.execute(text("INSERT INTO applications (job_id, full_name, email, linkedin_url, education, work_experience, resume_url) VALUES (:job_id, :full_name, :email, :linkedin_url, :education, :work_experience, :resume_url)"), 
               {'job_id':job_id, 
